# AKS価格レベル変更でダウンタイムが発生するかどうか

結論：ダウンタイムは発生しない
- ノードプール１つで確認
- 1秒おきに curl しながら確認
- 2️⃣まで実施して、ポータルで変更して確認
```bash
while true; do
  echo -n "$(date '+%Y-%m-%d %H:%M:%S JST') "
  curl -s -o /dev/null -w "%{http_code}\n" --max-time 1 http://130.33.237.150 ; sleep 1
done | tee -a status.log
```
- ログ：status.log
...
1. 無料 → Standard
    - 2025/09/27 15:28 - 15:32 (4分)
    - 接続断なし
2. Standard → LTS(Premium)
    - 2025/09/27 15:33 - 15:38 (5分)
    - 接続断なし
3. アップグレード 1.30.14 → 1.31.11
    - 2025/09/27 15:39 - 15:55
    - 15:46:01〜15:46:32まで不通
      - ノードプールが1つなのが原因
4. LTS(Premium) → Standard
    - 2025/09/27 15:56 - 15:58 (2分)
    - 接続断なし
